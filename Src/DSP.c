...
void dsp(int16_t* buffer_input, int16_t* buffer_outputA, int16_t* buffer_outputB, uint8_t channel)
{
	...
	** ------------------------------------------------------------------- */
	arm_q15_to_float(buffer_input, inputf32, BLOCKSIZE);
	...
	** ------------------------------------------------------------------- */
	if (SCALE_INPUT != 1) {
		arm_scale_f32(inputf32, SCALE_INPUT,inputf32, BLOCKSIZE);
	}
	
	/* ----------------------------------------------------------------------
	** Apply F1 to the input
	** ------------------------------------------------------------------- */
	arm_biquad_cascade_df1_f32(F1, inputf32, inputf32, BLOCKSIZE);

	/* ----------------------------------------------------------------------
	** Apply F2 to the result - creates a new filter "branch"
	** ------------------------------------------------------------------- */
	arm_biquad_cascade_df1_f32(F2, inputf32, outputf32, BLOCKSIZE);

	if (SCALE_OUTPUT1_3 != 1) {
		arm_scale_f32(outputf32, SCALE_OUTPUT1_3,outputf32, BLOCKSIZE);
	}

	if (INVERT_OUTPUT1_3) {
		arm_negate_f32 (outputf32,outputf32, BLOCKSIZE);
	}

	/* ----------------------------------------------------------------------
	** Apply F3 to the result of F1 - prolonging the existing filter "branch"
	** ------------------------------------------------------------------- */
	arm_biquad_cascade_df1_f32(F3, inputf32, inputf32, BLOCKSIZE);

	if (SCALE_OUTPUT2_4 != 1) {
		arm_scale_f32(inputf32, SCALE_OUTPUT2_4,inputf32, BLOCKSIZE);
	}

	if (INVERT_OUTPUT2_4) {
		arm_negate_f32 (inputf32,inputf32, BLOCKSIZE);
	}
	...
	** ------------------------------------------------------------------- */
	arm_float_to_q15(inputf32,  buffer_outputA, BLOCKSIZE);
	arm_float_to_q15(outputf32, buffer_outputB, BLOCKSIZE);	
